...
| 10 | ✅ done | Live view overlay UX: fullscreen button in the detail modal, auto-reconnect on HLS stream error, stream status (live/reconnecting/error) surfaced in the modal. Design below. Extended 2026-08-17: in browser fullscreen specifically, `.overlay-side` (name/status/latency, action buttons, PTZ) no longer sits as a permanent stripe beside the video — it collapses into a single-line, semi-transparent bottom bar that's hidden until mouse movement and auto-hides after ~2.5s idle (`static/index.css`/`index.js`, `feature/fullscreen-overlay-bar`), so the video actually gets the whole screen. PTZ collapses from the 3x3 pad into one row of arrows; the name/latency/status cluster is pinned to the bar's right edge independently of the action buttons so a longer/shorter camera name can't shift Prev/Next's position; the bar has a fixed height so PTZ-enabled and analog (no-PTZ) channels render the same bar size. |
